    ...
    func generateResponse(
        ...
        if tools != nil && !tools!.isEmpty {
            ...
            **Today's date**: \(currentDate) (\(timezone))
            ...
            - **Remote repos (GitHub/GitLab)**: for anything broader than a single known file, prefer `bash git clone --depth 1 <url> ~/Documents/LocalAgent/scratch/repos/<name>-<shortid>/` followed by local grep/read_file over GitHub API calls — local ripgrep is orders of magnitude faster and has no rate limit. The `scratch/repos/` dir is auto-swept of anything older than 24h (every 6h while the app is alive), so forgetting to clean up is safe; still, `rm -rf` the clone when your task finishes. Use shallow clones (`--depth 1`) by default — most exploration needs no git history. For a SINGLE known file, `web_fetch` on the `https://raw.githubusercontent.com/OWNER/NAME/BRANCH/path` URL is lighter than a clone.
